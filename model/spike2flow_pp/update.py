...
class BasicMotionEncoder(nn.Module):
    def __init__(self, args):
        ...
        self.conv1 = nn.Conv2d(48+160, 96-2, 3, padding=1)
...
class BasicUpdateBlock(nn.Module):
    def __init__(self, args, hidden_dim=128, input_dim=128):
        ...
        self.gru = SepConvGRU(hidden_dim=hidden_dim, input_dim=96*3+input_dim+96)
    ...
    def forward(self, net, inp, corr_list, flow_list, corr_feat, upsample=True):
        corr1, corr2, corr3 = corr_list
        flow1, flow2, flow3 = flow_list

        mf1 = self.encoder(flow1, corr1)
        mf2 = self.encoder(flow2, corr2)
        mf3 = self.encoder(flow3, corr3)

        inp = torch.cat([inp, mf1, mf2, mf3, corr_feat], dim=1)
