...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x) -> Tuple[torch.Tensor, Size_]:
        ...
        patch_coord = coords_grid(B, out_size[0], out_size[1], x.device) * self.patch_size + self.patch_size/2 # in feature coordinate space
...
class Corr_Encoder(nn.Module):
    def __init__(self, patch_size=8, cost_heads_num=1, cost_latent_input_dim=64, pe='linear', cost_latent_token_num=8, cost_latent_dim=128, dropout=0.0):
        super(Corr_Encoder, self).__init__()
        self.patch_size = patch_size
        self.cost_heads_num = cost_heads_num
        self.patch_embed = PatchEmbed(in_chans=self.cost_heads_num, patch_size=self.patch_size, embed_dim=cost_latent_input_dim, pe=pe)

        # self.depth = cfg.encoder_depth
        self.latent_tokens = nn.Parameter(torch.randn(1, cost_latent_token_num, cost_latent_dim))

        query_token_dim, tgt_token_dim = cost_latent_dim, cost_latent_input_dim*2*3
        qk_dim, v_dim = query_token_dim, query_token_dim
    ...
    def forward(self, cost_volume_list, context=None):
        corr1, corr2, corr3 = cost_volume_list
        B, heads, H1, W1, H2, W2 = corr1.shape
        
        corr1 = corr1.permute(0, 2, 3, 1, 4, 5).contiguous().view(B*H1*W1, self.cost_heads_num, H2, W2)
        corr2 = corr2.permute(0, 2, 3, 1, 4, 5).contiguous().view(B*H1*W1, self.cost_heads_num, H2, W2)
        corr3 = corr3.permute(0, 2, 3, 1, 4, 5).contiguous().view(B*H1*W1, self.cost_heads_num, H2, W2)

        x1, size = self.patch_embed(corr1)   # B*H1*W1, size[0]*size[1], C
        x2, size = self.patch_embed(corr2)   # B*H1*W1, size[0]*size[1], C
        x3, size = self.patch_embed(corr3)   # B*H1*W1, size[0]*size[1], C

        x = self.input_layer(self.latent_tokens, torch.cat([x1, x2, x3], dim=2))
...
if __name__ == '__main__':
    patch_embed = PatchEmbed(in_chans=1, patch_size=4, embed_dim=128, pe='linear')

    cost_volume = torch.rand(4, 1, 48, 48, 48, 48)
    
    B, heads, H1, W1, H2, W2 = cost_volume.shape
    cost_maps = cost_volume.permute(0, 2, 3, 1, 4, 5).contiguous().view(B*H1*W1, 1, H2, W2)

    x, size = patch_embed(cost_maps)   # B*H1*W1, size[0]*size[1], C
    print(x.shape)