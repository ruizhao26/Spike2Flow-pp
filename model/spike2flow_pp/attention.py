...
def LinearPositionEmbeddingSine3D(x, dim=128, NORMALIZE_FACTOR=1/200):
    freq_bands = torch.linspace(0, dim // 6 - 1, dim // 6).to(x.device)
    sin_x = torch.sin(3.14 * x[..., 0:1] * freq_bands * NORMALIZE_FACTOR)
    cos_x = torch.cos(3.14 * x[..., 0:1] * freq_bands * NORMALIZE_FACTOR)
    sin_y = torch.sin(3.14 * x[..., 1:2] * freq_bands * NORMALIZE_FACTOR)
    cos_y = torch.cos(3.14 * x[..., 1:2] * freq_bands * NORMALIZE_FACTOR)
    sin_t = torch.sin(3.14 * x[..., 2:3] * freq_bands * NORMALIZE_FACTOR)
    cos_t = torch.cos(3.14 * x[..., 2:3] * freq_bands * NORMALIZE_FACTOR)
    
    return torch.cat([sin_x, cos_x, sin_y, cos_y, sin_t, cos_t], dim=-1)

if __name__ == '__main__':
    x = torch.rand(2800, 1, 2)
    out = LinearPositionEmbeddingSine(x=x, dim=128)
    print(out.shape)